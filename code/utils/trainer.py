    ...
    def _shared_loop(
        ...
        for batch_idx, batch in pbar:
            ...
            if mode == "train":
                ...
                if self.model.model.__class__.__name__ == "IIBMIL":
                    if self.update_prot_counter == 20:
                        with torch.no_grad():
                            self.model.model.update_prototypes(batch["X"], proto_m=0.6)
                        self.update_prot_counter = 0
                    else:
                        self.update_prot_counter += 1
            ...
            if batch_idx < (len(dataloader) - 1):
                pbar.set_postfix(
                    {
                        f"{mode}/{loss_name}": loop_loss_dict[loss_name].compute().item()
